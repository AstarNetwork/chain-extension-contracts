...
describe('DAPPS STAKING', () => {
    ...
    it('should nomination transfer', async () => {
        ...
        // @ts-ignore
        expect(stakerInfoContract1After.stakes[stakerInfoContract1Before.stakes.length - 1].staked.toBn()).to.equal(stakingAmount.sub(transferAmount).sub(minimumRemainingAmount))
        expect(stakerInfoContract2After.stakes[stakerInfoContract1Before.stakes.length - 1].staked.toBn()).to.equal(transferAmount)
    })
})