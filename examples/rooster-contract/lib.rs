...
pub mod governor {
    ...
    impl Governor {
        ...
        fn _emit_delegate_changed(&self, delegator: AccountId, to_delegate: AccountId, from_delegate: AccountId) {
        ...
        fn _emit_delegate_votes_changed(&self, delegate: AccountId) {
            ...
            self.env().emit_event(DelegateVotesChanged { delegate, votes })
        }

        fn _emit_collection_created(&self, collection_id: CollectionId, symbol: String) {
            self.env().emit_event(CollectionCreated { collection_id, symbol })
        }
        ...
        fn _cast_vote(&mut self, proposal_id: OperationId, vote: VoteType) -> Result<(), GovernorError> {
        ...
        fn _create_collection_metadata(&mut self, metadata: String, symbol: String) -> Result<(), GovernorError> {
            ...
            Rmrk::create_collection(metadata.into_bytes(), None, symbol.clone().into_bytes())
        ...
        fn _evolve_owner(&mut self, account: AccountId) -> Result<(), GovernorError> {
            ...
            if cur_lvl > 0 {
                let _result = Rmrk::remove_resource(self.collection_id.unwrap(), nft_id, cur_lvl - 1);
            }
            ...
            Rmrk::add_basic_resource(self.collection_id.unwrap(), nft_id, resource, cur_lvl)
        ...
        #[ink(message)]
        pub fn cast_vote(&mut self, proposal_id: OperationId, vote: VoteType) -> Result<(), GovernorError> {
        ...
        #[ink(message)]
        pub fn propose(&mut self, transaction: Transaction, description: String) -> Result<OperationId, GovernorError> {
            let caller = self.env().caller();
